      ...
      1: [
        function (require, module, exports) {
          ...
          function parse(state) {
            ...
            if (
              state.src.charCodeAt(pos) !== CHAR_OPEN_CURLY
            ) {
              return false;
            }
            ...
            while (
              state.src.charCodeAt(pos) !== CHAR_CLOSE_CURLY
            ) {
              if (
                !isValidClassChar(state.src.charCodeAt(pos))
              ) {
                return false;
              }
            ...
            if (
              pos !== state.posMax &&
              state.src.charCodeAt(pos) !== CHAR_NEWLINE
            ) {
              return false;
            }
            ...
            if (
              state.src.charCodeAt(initialPos - 1) ===
              CHAR_NEWLINE
            ) {
              preferOuter = true;
            }
            ...
            for (
              i = state.tokens.length - 1;
              i >= 0;
              i -= 1
            ) {
              if (
                state.tokens[i].type === "em_close" ||
              ...
              if (
                state.tokens[i].type === "em_open" ||
                state.tokens[i].type === "strong_open"
              ) {
                state.tokens[i].attrPush([
                  "class",
                  classString,
                ]);
                ...
                if (
                  pendingText.charCodeAt(
                    pendingText.length - 1
                  ) === CHAR_SPACE
                ) {
                  state.pending = pendingText.substring(
                    0,
                    pendingText.length - 1
                  );
                }
          ...
          function getClassyFromInlineToken(inlineToken) {
            ...
            if (
              tokens[numChildren - 1].type !== "classy" ||
              tokens.length === 1
            ) {
              return null;
            }
            ...
            if (
              tokens[numChildren - 1].type === "softbreak"
            ) {
              ...
              tokens[numChildren - 1].content =
                tokens[numChildren - 1].content.trim();
            }

            return classy;
          }

          function getOpeningToken(
            tokens,
            preferOuter,
            currentIndex
          ) {
            ...
            if (
              ...
              /_close$/.test(
                tokens[closingTokenIndex + 1].type
              )
            ) {
              closingTokenIndex += 1;
            }

            openingTokenType = tokens[
              closingTokenIndex
            ].type.replace("_close", "_open");

            for (i = currentIndex; i >= 0; i -= 1) {
              if (
                tokens[i].type === openingTokenType &&
                tokens[i].level ===
                  tokens[closingTokenIndex].level
              ) {
                return tokens[i];
              }
            }
          ...
          function parseBlock(state) {
            var i, openingToken, classy;

            for (i = 0; i < state.tokens.length; i += 1) {
              if (state.tokens[i].type === "inline") {
                classy = getClassyFromInlineToken(
                  state.tokens[i]
                );
                while (classy) {
                  openingToken = getOpeningToken(
                    state.tokens,
                    classy.preferOuter,
                    i
                  );
                  openingToken.attrPush([
                    "class",
                    classy.content,
                  ]);
